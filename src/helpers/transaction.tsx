...
const sendTransaction = async (
  ...
  const vts = await Promise.all(
    txs.map((tx) =>
      createVersionedTransaction(connection, tx, signers, wallet.publicKey, priorityFee, atas)
    )
  );
  const signedTransactions = await wallet.signAllTransactions(vts.map((vt) => vt.transaction));
...
export const executeMultipleTxs = async (
  connection: Connection,
  txs: { ixs: TransactionInstruction[][]; description: string; signers?: Signer[] }[],
  wallet: AnchorWallet
) => {
  invariant(wallet.publicKey);
...
export const executeJitoBundle = async (
  ...
  const launchpadLut = (await connection.getAddressLookupTable(LAUNCHPAD_LUT)).value;
  ...
  const vts = await Promise.all(
    txs.map(async (ixs) => {
      const messageV0 = new TransactionMessage({
        ...
        [launchpadLut].filter((x): x is AddressLookupTableAccount => x !== null)
      );
      ...
      transaction.sign(
        signers.filter((s) => txSigners.map((x) => x.toString()).includes(s.publicKey.toString()))
      );
  ...
  if (mode === 'jito') {
    ...
    for (const tx of vts) {
      ...
      await connection.confirmTransaction({ signature: hash, ...blockhash }, 'finalized');
      console.log(`https://solscan.io/tx/${hash}`);
    }
    return 'done';
  }
